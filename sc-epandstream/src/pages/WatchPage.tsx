  ...
  useEffect(() => {
    ...
    fetchEpisode(episodeId)
      .then(ep => {
        ...
        const firstQualityWithServers =
          qualities.find(q => q.serverList.some(s => s.isDefault)) ||
          qualities.find(q => q.serverList.length > 0);
        if (firstQualityWithServers) {
          setActiveQuality(firstQualityWithServers.title);
          const defaultServer =
            firstQualityWithServers.serverList.find(s => s.isDefault) ||
            firstQualityWithServers.serverList[0];
