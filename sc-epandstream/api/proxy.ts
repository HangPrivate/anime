...
/** `data-content` (base64) -> serverId aman untuk path URL (base64url). Kebalikannya ada di resolveStream(). */
function toServerId(data: string): string {
...
/** Server pertama yang isDefault=true beserta resolusinya. */
function findDefaultMirror(mirrors: Mirror[]): { resolution: string; service: MirrorService } | null {
  for (const m of mirrors) {
    const service = m.services.find((s) => s.isDefault);
    if (service) return { resolution: m.resolution, service };
  }
  return null;
}
...
function adaptEpisode(r: WatchScrape, meta: InfoScrape | null): EpisodeData {
  ...
  for (const m of r.mirrors) {
    ...
    for (const s of m.services) {
      ...
      const serverId = toServerId(s.data);
      // isDefault dari scraper diteruskan ke WatchPage; urutan resolusi & server = urutan di situs
      q.serverList.push({ title: s.service, serverId, href: `/server/${serverId}`, isDefault: s.isDefault });
    }
  }
  const qualities = Array.from(byQuality.values());
...
async function route(path: string): Promise<RouteResult> {
  ...
  switch (head) {
    ...
    case 'episode': {
      ...
      let r = unwrap(await watch(`${SOURCE}/episode/${arg}/`));
      // Kalau halaman tidak menyertakan iframe default, ambil stream dari server isDefault
      if (!r.embedIframe) {
        const def = findDefaultMirror(r.mirrors);
        const url = def ? await withTimeout(resolveStream(toServerId(def.service.data)), 3000) : null;
        if (url) r = { ...r, embedIframe: url };
      }
